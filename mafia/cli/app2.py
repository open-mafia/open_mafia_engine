...
class CliApp(object):
    ...
    def __init__(self):
        api_args = ["--host", "localhost", "--port", "8000", "--log_level", "warning"]
        self.api_process = Process(target=api_main, args=[api_args], daemon=True)
        ...
        while True:
            try:
                ...
                if r == "back":
                    ...
                    for name in to_add:
                        requests.post(url + "/add?player_name={}".format(name))
                elif r == "remove":
                    p_rem = PromptSession(
                        message="name: ",
                    ...
                    for name in to_remove:
                        requests.post(url + "/remove?player_name={}".format(name))
                else:
                    print(r)

    def print_player_info(self):
        all_info = requests.get(self.base_url + "/players/all").json()
        for pi in all_info:
                ...
                if self.game_started:
                    ...
                    if winner:
                        ...
                        w_players = requests.get(
                            self.base_url + "/players?team_name={}".format(winner)
                        ).json()
                        ...
                        options = ["back", "next-phase", "players", "ability", "end"]
        ...
        while True:
            try:
                ...
                if r == "back":
                    ...
                    requests.post(
                        url
                        + "/{}?ability_name={}&target={}".format(r, active_abil, targ)
                    )
                    if phase == "night":
                        self.bump_phase()
